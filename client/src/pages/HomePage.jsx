...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen">
      {/* ── HERO ── */}
      <section className="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden bg-mesh">
        {/* Decorative background lights */}
        <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-eco-500/5 rounded-full blur-3xl pointer-events-none" />
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-cyan-500/5 rounded-full blur-3xl pointer-events-none" />

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-12 gap-12 items-center relative z-10 w-full">
          <div className="lg:col-span-7 animate-fade-up">
            ...
            <p className="text-lg text-eco-700 mb-8 max-w-xl leading-relaxed">
            ...
            {/* Amazon-style search block */}
            <form onSubmit={handleSearch} className="glass-card p-2 flex items-center max-w-2xl border-white/10 hover:border-eco-500/30 transition-all duration-300 mb-8 shadow-2xl">
              <div className="relative flex-1 flex items-center pl-3">
                <Search className="w-5 h-5 text-eco-700 shrink-0" />
                <input
                  type="text"
                  placeholder="Search materials (e.g. Aluminium, Plastics, Wood)..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full bg-transparent border-none outline-none text-eco-100 placeholder-eco-800 px-3 py-3 text-base"
                />
              </div>
              <button type="submit" className="btn-primary py-3 px-6 flex items-center gap-2 text-base shrink-0">
                Explore <ArrowRight className="w-4 h-4" />
              </button>
            </form>

            <div className="flex flex-wrap items-center gap-6 text-sm text-eco-700">
              <span className="flex items-center gap-2"><Building2 className="w-4 h-4 text-eco-500" /> 100% Verified Businesses</span>
              <span className="flex items-center gap-2"><Flame className="w-4 h-4 text-eco-500" /> Live bidding enabled</span>
            </div>
          </div>

          {/* Graphical circular visual representing waste circularity */}
          <div className="lg:col-span-5 relative flex justify-center items-center h-[450px]">
            ...
            {/* Orbiting nodes representing marketplace steps */}
            {['⚙️', '🧪', '📦', '💻', '♻️'].map((emoji, i) => (
              <div
                key={i}
      ...
      {/* ── CATEGORIES NAV ── */}
      <section className="py-12 border-t border-white/[0.06] bg-dark-400/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="font-display text-2xl font-bold">Featured Categories</h2>
              <p className="text-sm text-eco-700">Quickly filter waste materials by category</p>
            </div>
            <Link to="/marketplace" className="text-sm text-eco-400 hover:text-eco-200 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {CATEGORIES.map((cat) => (
              <Link
                ...
                className={`glass-card p-5 text-center bg-gradient-to-b ${cat.color} border border-white/[0.06] ${cat.border} transition-all duration-300 hover:-translate-y-1 block`}
              >
                <span className="text-3xl block mb-3">{cat.emoji}</span>
                <span className="text-sm font-semibold text-eco-200 block truncate">{cat.name}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-white/[0.06]">
        <div className="text-center mb-16">
          <div className="section-tag"><Package className="w-3.5 h-3.5" /> Real-world Listings</div>
          <h2 className="font-display text-4xl font-bold mt-2">Recently Listed Materials</h2>
          <p className="text-eco-700 mt-3 max-w-xl mx-auto">Explore actual corporate listings available for active trade and transaction bidding.</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((s) => (
            <div key={s.label} className="text-center">
              ...
              <div className="text-sm text-eco-700 font-medium">{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-4xl font-bold mt-2">Built for Industrial Commerce</h2>
          <p className="text-eco-700 mt-3 max-w-xl mx-auto">Six powerful modules working in sync to power circular supply chains at scale.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f) => (
            <div key={f.title} className="glass-card p-6 hover:border-eco-500/30 hover:-translate-y-1 transition-all duration-300 group">
              ...
              <h3 className="font-display font-semibold mb-2 text-eco-200">{f.title}</h3>
      ...
      {/* ── HOW IT WORKS ─── */}
      <section id="how-it-works" className="py-24 bg-dark-400/40 border-t border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <div className="section-tag"><RefreshCw className="w-3.5 h-3.5" /> Process Flow</div>
            <h2 className="font-display text-4xl font-bold mt-2">How EcoLoop Connects Businesses</h2>
          </div>
          <div className="grid md:grid-cols-4 gap-6">
            {STEPS.map((s, i) => (
              <div key={s.n} className="glass-card p-6 relative">
                ...
                <h3 className="font-display font-semibold mb-2 text-eco-200">{s.title}</h3>
                <p className="text-sm text-eco-700 leading-relaxed">{s.desc}</p>
                {i < STEPS.length - 1 && (
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 text-center">
        <div className="glass-card max-w-3xl mx-auto p-12 border-eco-500/20 relative overflow-hidden shadow-2xl">
          <div className="absolute inset-0 bg-gradient-to-br from-eco-500/5 to-transparent pointer-events-none" />
          <Leaf className="w-12 h-12 text-eco-400 mx-auto mb-5 animate-pulse" />
          <h2 className="font-display text-3xl lg:text-4xl font-bold mb-4">Ready to Close the Loop?</h2>
          <p className="text-eco-700 mb-8 max-w-md mx-auto">Join over 1,400+ industrial members who list, match, and trade circular materials today.</p>
      ...
      <footer className="border-t border-white/[0.06] py-8 px-4">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-eco-800">
          ...
          <p>© 2026 EcoLoop. B2B Circular Economy Platform.</p>
